...
I first started using [ES6](https://github.com/lukehoban/es6features) and [Webpack](https://webpack.github.io/) a few months whilst working on [Soapee](http://soapee.com) and ever since
 I've been finding myself not looking forward to using ES5 and other packagers for Node.js and AngularJS applications.

This posts presents my findings on my reasearch on existing ES6/AngularJS seed projects and my own attempts of using ES6 and Webpack on an AngularJS single page application.

## Similar Resources

Googling for [angular webpack](https://www.google.co.uk/search?q=angular+webpack) yields several resources and blog posts where
authors have documented their experiments with Webpack and and AngularJS. Notable entries include:

### Egghead.io

Kent Dodds put together an interesting series of videos on [Egghead](https://egghead.io) called [Angular with Webpack](https://egghead.io/lessons/angularjs-angular-with-webpack-introduction) which go
through initial Webpack configuration, ES6 and Babel, pre-processors, production and testing. Please note that the first of these videos is free to view while the rest require **premium** membership.
...
covers the basics and talks about project folder structures, directives, services,testing and so on.
...
My motivation for [angular-es6-webpack-seed](https://github.com/nazar/angular-es6-webpack-seed) was to build a seed project based on my previous experience on building
 a large Angular single page application. I also wanted to exploit the productivity gains introduced by Webpack and the following tools and libraries:
...
+ css hot loading. Not as awesome as awesome as [react-hot-loader](https://github.com/gaearon/react-hot-loader) wit React but *great* nonetheless
+ support for AMD/CommonJS module types - but namely ES6's `import from xyz 'xyz'` syntax
+ adoption of component based folder structures for controllers views and directives that group files by functionality. More on this later
...
documentation to quickly setup an isolated virtual development environment.
...
through which all ports are accessible (i.e. port 22 for SSH logins and port 8080 for the development web server). Vagrant typically maps VM ports to the local
port (i.e. VM:8080 to local:80 and VM:22 to local: 2222) but this could become an issue that prevents multiple Vagrant VMs from running.
...
+ **/src** - this is the single page application's root folder. If using **WebStorm**, set this folder as the Resource Root (under settings -> Project -> Directories)
  ...
  + **/src/app** - the AngularJS single application folder which defines the Angular [boot](https://github.com/nazar/angular-es6-webpack-seed/blob/master/src/app/index.js)
 and ui-routes based [routes](https://github.com/nazar/angular-es6-webpack-seed/blob/master/src/app/routes.js) files
...
This is the application boot file; it defines the project's dependencies and routes.

#### [src/app/routes.js](https://github.com/nazar/angular-es6-webpack-seed/blob/master/src/app/routes.js)

This is the [ui-router](https://github.com/angular-ui/ui-router) based routes file and defines the following nested routes:
...
+ **app** - defines the application root and is linked to the ApplicationController and ApplicationTemplate and defines the application layout.
  ...
  users will be redirected to **app.login**. This is where **ui-router** shines as we can define nested **app.my** views that all *inherit* the authenticationn check
   from **app.my**
    + **app.my.profile** - a logged in user's profile page. Only accessible for logged in users

#### [controllers](https://github.com/nazar/angular-es6-webpack-seed/tree/master/src/app/controllers)

Controllers define distinct views - i.e. ***page1***, ***page2***, ***login***. Each controller is contained in its own folder and includes:

+ index.js - the exported controller function
+ Style definitions
+ HTML template
+ Images

The [myProfile](https://github.com/nazar/angular-es6-webpack-seed/tree/master/src/app/controllers/myProfile) best demonstrates the above
...
`index.js` is responsible for importing the controller's style and images. The controller's template, defined in the routes.js file is:
...
Note the namespaced view: `<div id="my-profile">`. The controller's style [file](/src/app/controllers/myProfile/style.styl) uses the same `#my-profile` namespace for all controller styles.
...
Again, the root **services** folder contains an **index.js** file which imports all services files into the application module.

{% codeblock auth.js lang:javascript %}
...
+ **livereload** for any JavaScript file changes - functional but as awesome as React's hotloading
+ **hot loading** for CSS style and image file changes - these updates do not require a page refresh

The development server is started by issuing the `gulp` command in the project's root folder.

### Building and Deploying

The [base](https://github.com/nazar/angular-es6-webpack-seed/blob/master/webpack.config.js) webpack configuration is re-used by the
